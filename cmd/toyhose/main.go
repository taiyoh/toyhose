...
func main() {
	...
	awsConf, err := config.LoadDefaultConfig(context.Background(),
		config.WithDefaultRegion(conf.Region),
		config.WithCredentialsProvider(credentials.NewStaticCredentialsProvider(conf.AccessKeyID, conf.SecretKey, "")))
	if err != nil {
		log.Fatal().Err(err).Msg("failed to load AWS config")
	}
	d := toyhose.NewDispatcher(context.Background(), &toyhose.DispatcherConfig{
		AWSConf: &awsConf,
